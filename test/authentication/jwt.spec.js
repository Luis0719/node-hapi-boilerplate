...
describe('#jwt', function () {
  ...
  before(async function() {
    ...
    const guest_role = await factories.Role.create({ name: 'guest', actions: [public_action.id] });
  ...
  describe('validate', function() {
    it('should fail if no id is provided',  async function() {
      ...
      return expect(validate(payload, request)).to.be.rejectedWith(/Invalid authorization key/);
    });
    ...
    it('should fail for unexisting user', async function() {
      ...
      return expect(validate(payload, request)).to.be.rejectedWith(/User not authorized/);
    });
